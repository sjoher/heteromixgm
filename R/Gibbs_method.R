  ...
  calculate_EM_Gibbs = function(combination, y, lambda1 = lambdas[1,1], lambda2 = lambdas[1,2], Theta=NULL, lower_upper, em.tol=.001, em.iter=10, c.em.iter=1, gibbs.iter=1000, mc.iter=1000, ncores)
  {
    ...
    while((c.em.iter <= em.iter) && (difsum >= (K*em.tol)))
    {
      ...
      for(k in 1:K){
        ...
        Theta[[k]] = as(as(as(R.fgl$theta[[k]], "dMatrix"), "generalMatrix"), "TsparseMatrix")
    ...
    loglik <- numeric(K)
    for(k in 1:K){
      loglik[k] <- n_k[k]/2 *(determinant(as.matrix(Theta[[k]]), logarithm = TRUE)$modulus - sum(diag(R[[k]] %*% as.matrix(Theta[[k]]))))
      #loglik[k] <- n_k[k]/2 *(log(det(Theta[[k]])) - sum(diag(R[[k]] %*%Theta[[k]])))
    }
    loglik <- sum(loglik)
